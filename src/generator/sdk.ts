import {OAS, OASOperation, OASParameter, OASPath, OASRef, OASRequestBody, OASResponse} from '../oas';
import fs from 'fs';
...
function sdkMethod(path: string, method: string, pathParameters: string[], queryParameters: string[], headerParameters: string[], oas: OAS, methodDefinition: OASOperation, imports: string[]): string {
  ...
  return `    async ${methodName}(${params.join(', ')}): Promise<(${returnType(oas, methodDefinition, imports)}) & {headers: Record<string, string>}>{
      ...
      const result = await this.caller.call('${method.toUpperCase()}', resource, path, ${bodyParam ? 'JSON.stringify(body)' : 'undefined'}, ${pathParams ? 'params' : '{}'}, ${queryParams ? 'queryParameters' : '{}'}, ${multiQueryParams ? 'multiQueryParameters' : '{}'}, headers);
  ...
  const sdkMethods = paths.flatMap(({ path, definition }) => {
    if (definition) {
      return methodOperations(definition).map(({ method, definition: operation }) => {
        ...
        return sdkMethod(path, method, pathParameters, queryParameters, headerParameters, schema, operation, imports);
      });
    } else return [];
  });
