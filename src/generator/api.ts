...
export async function api(schema: OAS): Promise<string> {
  const operations = Object.keys(schema.paths).flatMap((path) => Object.keys(schema.paths[path]).map((operation) => ({ operationId: schema.paths[path][operation].operationId, operation, path })))
  return `
...
export interface ${schema.info.title.replace(/ /g, '')}Handlers {
    ${operations.map(({ operationId }) => `${operationId}: (request: APIGatewayProxyEvent) => Promise<APIGatewayProxyResult>;`).join("\n    ")}
}
...
export class ${schema.info.title.replace(/ /g, '')} implements ApiHandler {
    ...
    apiRoutes() {
      return [${operations.map(({ operationId, operation, path }) => `
